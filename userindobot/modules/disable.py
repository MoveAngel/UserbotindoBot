...
from userindobot.modules.helper_funcs.handlers import CMD_STARTERS, CustomCommandHandler
from userindobot.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from userindobot.modules.helper_funcs.chat_status import user_admin, is_user_admin, connection_status
    from userindobot.modules.sql import disable_sql as sql
    ...
    def disable_module(bot: Bot, update: Update, args: List[str]):
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "userindobot.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            enable_module = "userindobot.modules." + args[0].rsplit(".", 1)[0]
