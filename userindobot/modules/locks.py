...
import userindobot.modules.sql.locks_sql as sql
from userindobot import dispatcher, SUDO_USERS, DEV_USERS, LOGGER
...
LOCK_TYPES = {
    ...
    'url': Filters.entity(
        MessageEntity.URL) | Filters.caption_entity(
            MessageEntity.URL),
...
def del_lockables(bot: Bot, update: Update):
    ...
    for lockable, filter in LOCK_TYPES.items():
        if filter(message) and sql.is_locked(
                chat.id,
