...
from userindobot.modules.helper_funcs.string_handling import extract_time

import userindobot.modules.sql.blsticker_sql as sql
from userindobot import dispatcher, LOGGER
from userindobot.modules.disable import DisableAbleCommandHandler
from userindobot.modules.helper_funcs.chat_status import user_not_admin, user_admin
from userindobot.modules.helper_funcs.misc import split_message
from userindobot.modules.warns import warn
from userindobot.modules.log_channel import loggable
from userindobot.modules.connection import connected
    ...
    for text in split_text:
        if sticker_list == "<b>List black stickers currently in {}:</b>\n".format(
            ...
            send_message(
                ...
                "There is no blacklist sticker in <b>{}</b>!".format(chat_name),
...
def blacklist_mode(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        if args[0].lower() in ['off', 'nothing', 'no']:
            ...
            return
        if conn:
            text = "Blacklist sticker mode changed, will `{}` at *{}*!".format(
                settypeblacklist, chat_name)
        else:
            text = "Blacklist sticker mode changed, will `{}`!".format(
                settypeblacklist)
            ...
            try:
                if getmode == 0:
                    ...
                    warn(
                        ...
                        chat,
                        "Using sticker '{}' which in blacklist stickers".format(trigger),
                        message,
                        update.effective_user)
                    return
                elif getmode == 3:
