...
from userindobot import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, SARDEGNA_USERS, GBAN_LOGS, LOGGER
from userindobot.modules.helper_funcs.chat_status import is_user_admin
from userindobot.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from userindobot.modules.helper_funcs.string_handling import markdown_parser
from userindobot.modules.disable import DisableAbleCommandHandler

import userindobot.modules.sql.feds_sql as sql
from userindobot.modules.helper_funcs.alternate import send_message
...
@run_async
def del_fed(_bot: Bot, update: Update, args: List[str]):
    chat = update.effective_chat
    user = update.effective_user
    message = update.effective_message
    if chat.type != "private":
...
@run_async
def fed_chat(_bot: Bot, update: Update, _args: List[str]):
    chat = update.effective_chat
...
@run_async
def leave_fed(bot: Bot, update: Update, _args: List[str]):
    chat = update.effective_chat
    user = update.effective_user
    message = update.effective_message
    if chat.type == 'private':
...
@run_async
def fed_admin(bot: Bot, update: Update, _args: List[str]):
    chat = update.effective_chat
    user = update.effective_user
    message = update.effective_message
    if chat.type == 'private':
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @userbotindo.")
            return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @userbotindo !")
            return
...
@run_async
def get_frules(_bot: Bot, update: Update, _args: List[str]):
    chat = update.effective_chat
    message = update.effective_message
    if chat.type == 'private':
        send_message(message,
...
@run_async
def fed_ban_list(_bot: Bot, update: Update, args: List[str], chat_data):
    chat = update.effective_chat
    user = update.effective_user
    message = update.effective_message
    if chat.type == 'private':
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "userindobot_fbanned_users.json"
                message.reply_document(
                    document=output,
                    filename="userindobot_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "userindobot_fbanned_users.csv"
                message.reply_document(
                    document=output,
                    filename="userindobot_fbanned_users.csv",
...
@run_async
def fed_notif(_bot: Bot, update: Update, args: List[str]):
    chat = update.effective_chat
    user = update.effective_user
    fed_id = sql.get_fed_id(chat.id)
    message = update.effective_message
...
@run_async
def fed_chats(_bot: Bot, update: Update, _args: List[str]):
    chat = update.effective_chat
    user = update.effective_user
    message = update.effective_message
    if chat.type == 'private':
...
@run_async
def del_fed_button(_bot, update):
...
@run_async
def set_fed_log(_bot, update, args):
    chat = update.effective_chat
    user = update.effective_user
    message = update.effective_message
    if chat.type == 'private':
...
@run_async
def unset_fed_log(_bot, update, args):
    chat = update.effective_chat
    user = update.effective_user
    message = update.effective_message
    if chat.type == 'private':
...
@run_async
def get_myfeds_list(_bot, update):
    user = update.effective_user
    message = update.effective_message
...
@run_async
def fed_admin_help(_bot: Bot, update: Update):
    message = update.effective_message
...
@run_async
def fed_user_help(_bot: Bot, update: Update):
    message = update.effective_message
