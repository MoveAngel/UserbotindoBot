...
from userindobot.__main__ import (
    IMPORTED,
    HELPABLE,
    MIGRATEABLE,
    STATS,
    ...
    CHAT_SETTINGS,
    USER_SETTINGS)
from userindobot.modules.helper_funcs.chat_status import sudo_plus, dev_plus
...
def load(_bot: Bot, update: Update):
    ...
    try:
        imported_module = importlib.import_module("userindobot.modules." + text)
    except Exception:
...
def listmodules(_bot: Bot, update: Update):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("userindobot.modules.", 1)[1]
