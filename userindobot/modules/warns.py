...
from userindobot import dispatcher, BAN_STICKER, WHITELIST_USERS, SARDEGNA_USERS
from userindobot.modules.disable import DisableAbleCommandHandler
from userindobot.modules.helper_funcs.chat_status import (
    is_user_admin, bot_admin, user_admin_no_reply, user_admin, can_restrict)
from userindobot.modules.helper_funcs.extraction import extract_text, extract_user_and_text, extract_user
from userindobot.modules.helper_funcs.filters import CustomFilters
from userindobot.modules.helper_funcs.misc import split_message
from userindobot.modules.helper_funcs.string_handling import split_quotes
from userindobot.modules.log_channel import loggable
from userindobot.modules.sql import warns_sql as sql
...
def warn(
    ...
    if num_warns >= limit:
        ...
        message.bot.send_sticker(chat.id, BAN_STICKER)  # UserbotindoBot sticker
        keyboard = []
        log_reason = (
...
def warn_user(_bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id:
        if message.reply_to_message and message.reply_to_message.from_user.id == user_id:
            return warn(message.reply_to_message.from_user, chat,
                        reason, message.reply_to_message, warner)
        else:
